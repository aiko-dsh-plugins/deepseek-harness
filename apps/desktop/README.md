...
| Package sources | The exact dsh source build must be packageable before npm publication and install offline; plugins need independently published sources. | The signed application carries locally packed first-party dsh packages and an offline seed store. Desktop plugins use the fixed npm registry or explicitly named HTTPS release tarballs. |
...
The main dsh renderer receives the desktop protocol marker and a versioned `dshDesktop.market` bridge with `list()` and `request(proposal)`. A proposal installs an ordered `specs` list or removes one named plugin; a native dialog displays the exact packages and sources before any mutation. Only the application's main frame can propose changes, concurrent proposals are rejected, and cancellation leaves package state untouched. The separate plugin window retains its structured management operations. Neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.

Market installation accepts registry package names and `name@https://host/path/package.tgz`. Local paths, credentials, query strings, fragments, duplicate package names, and replacement of release-owned packages are rejected. Dependency batches activate together in their declared order; updating an installed dependency retains its position. Removal checks the remaining plugins' dependency and peer-dependency declarations. Remote sources remain in the manifest and lockfile and are reused during release reconciliation. The [market integration decision](../../.agents/notes/implemented/architecture/2026-09-10-desktop-market-proposals.md) describes the ownership tradeoff.
...
The [preinstalled plugin map](preinstalled-plugins.json) selects optional bundles for a new Desktop profile. Seed preparation downloads their exact published sources and includes them in the offline lockfile and store; this distribution preinstalls the Aiko market and Aiko Office. Office includes native Python and document libraries for Windows x64, macOS arm64 and macOS x64. These plugins remain removable through the native plugin manager. Restart and release reconciliation retain the active profile's plugin versions, sources, and removals, so a removed default is not installed again. An existing profile does not acquire new defaults from an application upgrade.

`DSH_DESKTOP_FETCH_TIMEOUT_MS` sets pnpm's download timeout to a positive integer in milliseconds; an absent value retains pnpm's default. Seed preparation can reuse a downloaded pnpm store from `DSH_DESKTOP_PREFETCHED_STORE` and a reviewed lockfile from `DSH_DESKTOP_PREFETCHED_LOCKFILE`. Core-package validation, frozen-lockfile installation, content integrity checks and the final offline installation still run. These inputs configure the build and are not copied into user settings.
...
4. During an Electron upgrade, read every plugin name, exact version, and remote artifact source from the old active profile, remove seed defaults absent from that profile, and restore its plugins to staging with `--offline` from existing Desktop pnpm state. A first installation keeps the seed defaults.
...
GUI plugin mutations use the same staging, health-check, activation, and rollback path after installing published packages into the shared Desktop pnpm store.
